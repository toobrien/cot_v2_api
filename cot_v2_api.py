...
def convenience_recs(report_type: int, data: dict):
    ...
    if report_type in [ report.disagg_futs_only, report.disagg_futs_and_opts ]:
        ...
        conv[disagg_futs_only.swap_long_pct]        = [ float(x)    for x in data[disagg_futs_only_raw.pct_of_oi_m_money_long_all]                                              ]
        conv[disagg_futs_only.swap_short]           = [ int(x)      for x in data[disagg_futs_only_raw.m_money_positions_short_all]                                             ]
        conv[disagg_futs_only.swap_short_pct]       = [ float(x)    for x in data[disagg_futs_only_raw.pct_of_oi_m_money_short_all]                                             ]
        conv[disagg_futs_only.swap_spread]          = [ int(x)      for x in data[disagg_futs_only_raw.pct_of_oi_m_money_spread_all]                                            ]
        conv[disagg_futs_only.swap_spread_pct]      = [ float(x)    for x in data[disagg_futs_only_raw.pct_of_oi_swap_spread_all]                                               ]
        conv[disagg_futs_only.swap_net]             = [ conv[disagg_futs_only.managed_long][i]      - conv[disagg_futs_only.managed_short][i]       for i in range(n_recs)      ]
        conv[disagg_futs_only.swap_net_pct]         = [ conv[disagg_futs_only.managed_long_pct][i]  - conv[disagg_futs_only.managed_short_pct][i]   for i in range(n_recs)      ]
